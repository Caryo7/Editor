...
class MenuBar:
    def __menu__(self):
        ...
        if get_menufile():
            ...
            self.menufichier.add_separator()
            self.menufichier.add_command(label=lg('settings'), accelerator=self.get_accelerator('settings'), stat = 'disabled', command = None, image = self.images['settings'], compound = 'left')
            self.menufichier.add_separator()
